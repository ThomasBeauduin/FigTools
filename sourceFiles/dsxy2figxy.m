...
if exist('x','var')     % Transform a and return pair of points
    if strcmp(axXscale,'linear')
        varargout{1} = (x-axlim(1))/axwidth  * axpos(3) + axpos(1);
    else
        varargout{1} = log(abs(x)./axlim(1))./log(axlim(2)/axlim(1)) * axpos(3) + axpos(1);
    end
    if strcmp(axYscale,'linear')
        varargout{2} = (y-axlim(3))/axheight * axpos(4) + axpos(2);
    else
        varargout{2} = log(abs(y)./axlim(3))./log(axlim(4)/axlim(3)) * axpos(4) + axpos(2);
    end
